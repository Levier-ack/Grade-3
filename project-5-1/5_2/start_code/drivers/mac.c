...
void check_recv(mac_t *test_mac)
{
    ...
    while(ch_flag <= 64){
        ...
        for(i = 0; i < 64; i++){
            printf("%x  ",*((uint32_t*)RECV_BUFFER + i + (ch_flag-1)*0x400/4));
        }
        if(ch_flag != 64)
...
uint32_t do_net_recv(uint32_t rd, uint32_t rd_phy, uint32_t daddr)
{
   //PLEASE enable MAC-RX
    int i = 0;
    uint32_t *desc_addr = (uint32_t *)rd;
    ...
    for(i = 0; i < 64; i++){//一次发送64个
        desc_addr[i*4] = 0x80000000;
    }
    // printkf("%x",*desc_addr);

    for(i = 0; i < 64; i++){
        reg_write_32(DMA_BASE_ADDR + 0x8, 0x4396);
    }

    // uint32_t *temp_desc_addr = desc_addr + 63*4;
    // while((temp_desc_addr[0] & 0x80000000) != 0x00000000);
...
void do_net_send(uint32_t td, uint32_t td_phy)
{
    ...
    int i = 0;
    uint32_t *desc_addr = (uint32_t*)td;
    ...
    while((desc_addr[1] & 0x02000000) != 0x02000000){
        reg_write_32(DMA_BASE_ADDR + 4, 0x4396);
        while((desc_addr[0] & 0x80000000) != 0x00000000){}
...
void do_wait_recv_package(mac_t *mac)
{
    mac_ptr = mac;
